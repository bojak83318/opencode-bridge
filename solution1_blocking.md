# Solution 1: Blocking Mode Bridge

## Overview
Use the default blocking behavior of `session.prompt()` which waits for the complete response.

## Key Insight
From research: `session.prompt()` WITHOUT `noReply: true` **blocks until response is complete** and returns the full `AssistantMessage` directly.
...
# 3. Start bridge
cd opencode-bridge && node opencode_bridge_solution1.js &
sleep 3
...
curl -s -X POST http://localhost:8083/v1/chat/completions \
  ...
  -d '{"model": "opencode/kimi-k2.5-free", "messages": [{"role": "user", "content": "What is 2+2?"}]}'
```

## Success Criteria
- Response contains non-empty `content` field
- No timeout (response within 30 seconds)
